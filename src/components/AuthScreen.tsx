...
export default function AuthScreen({ onLogin }: { onLogin: (user: any) => void }) {
  ...
  return (
    <div id="auth-screen" className="app-screen active" style={{ overflowY: 'auto', display: 'block' }}>
      <div style={{ minHeight: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '40px 0' }}>
        ...
        {isLoginMode ? (
          ...
          >
            <h2>로그인</h2>
            {errorMsg && <p className={`text-sm mb-4 text-center ${errorMsg.includes('완료') ? 'text-white' : 'text-red-400'}`}>{errorMsg}</p>}
