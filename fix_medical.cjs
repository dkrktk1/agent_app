...
// Fix the replaced part in showPainModal
const incorrectReplacement = `</div>
                    )}
...
const originalPart = `<div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-red-700"></span>5단계(극심한 통증)</div>
                          </div>
                        </div>
                      )}
...
// Restore the original string that was replaced accidentally
code = code.replace(incorrectReplacement, `<div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-red-700"></span>5단계(극심한 통증)</div>
                          </div>`);

// Now correctly apply it to the actual history modal which is lower down
const correctRegex = /<div className="flex items-center gap-1\.5"><span className="w-2\.5 h-2\.5 rounded-full bg-red-700"><\/span>5단계\(극심한 통증\)<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>/;

// Wait, the correct place was:
/*
...
*/
// We need to inject the code just before `</div> </div> </div> )}`
const correctTarget = `<div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-red-700"></span>5단계(극심한 통증)</div>
                    </div>
                  </div>
                </div>
              )}`;

const newTarget = `<div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-red-700"></span>5단계(극심한 통증)</div>
                    </div>
                  )}
...
code = code.replace(correctTarget, newTarget);
