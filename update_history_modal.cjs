...
// We should reset historyModalTab when closing the modal, or opening.
// find: useModalHistory(showHistoryModal, () => setShowHistoryModal(false));
code = code.replace(
...
// We need to also reset when opening via handleShowHistory or handleBodyPartPress.
// It's probably easier to just do it when rendering or rely on the fact it closes and state persists but we can reset on open.
code = code.replace(
...
// Replace the rendering inside the modal.
const modalStartRegex = /<div className="flex justify-between items-start mb-6">[\s\S]*?<div className="h-\[300px\] w-full mt-2">/;
                      ...
                      {painData[historyModalPart] && !painData[historyModalPart].isPast && (
                        <div className="flex p-1 bg-[rgba(255,255,255,0.05)] rounded-xl mt-2 w-full">
                          <button
                            ...
                            className={\`flex-1 py-2 text-sm font-bold rounded-lg transition-colors \${historyModalTab === 'graph' ? 'bg-[var(--primary-color)] text-[var(--card-bg)]' : 'text-gray-400 hover:text-white'}\`}
                          >
                            그래프
                          </button>
                          <button
                            onClick={() => setHistoryModalTab('card')}
                            className={\`flex-1 py-2 text-sm font-bold rounded-lg transition-colors \${historyModalTab === 'card' ? 'bg-[var(--primary-color)] text-[var(--card-bg)]' : 'text-gray-400 hover:text-white'}\`}
                          >
                            부상 내역 카드
                          </button>
                        </div>
...
// Add the closing brace for the graph section and render the card if tab is 'card'
// We look for:
//                      <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-red-700"></span>5단계(극심한 통증)</div>
//                    </div>
const tabsEndRegex = /<div className="flex items-center gap-1\.5"><span className="w-2\.5 h-2\.5 rounded-full bg-red-700"><\/span>5단계\(극심한 통증\)<\/div>\s*<\/div>/;
const cardRendering = `</div>
                    )}
                    
                    {historyModalTab === 'card' && painData[historyModalPart] && (
...
code = code.replace(tabsEndRegex, cardRendering);
