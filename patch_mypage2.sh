#!/bin/bash
cat << 'INNEREOF' > patch_my.txt
        </div>

        {!isAgent ? (
          <div className="flex gap-2 mt-4">
            <button className="btn-primary flex-1" onClick={() => handlePlayerClick({ ...activeUser, id: activeUser.id || activeUser.userId }, true)}>
              프로필 수정
            </button>
...
INNEREOF
sed -i -e '/<div className="info-item">/,/<\/div>         <\/div>          {!isAgent && (/!b' src/components/MyPageTab.tsx
